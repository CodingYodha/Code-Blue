document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("http://127.0.0.1:8000/get_ai_report")
    .then(async (res) => {
      if (!res.ok) {
      ...
      data.forEach((ai_report) => {
        ...
        row.innerHTML = `
          ...
          <td>
            <input type="checkbox" class="gov-verified-checkbox" data-uq-id="${ai_report.uq_id}" ${
      ...
      document.querySelectorAll(".gov-verified-checkbox").forEach((checkbox) => {
        checkbox.addEventListener("change", (event) => {
          const uqId = event.target.getAttribute("data-uq-id");
          const isChecked = event.target.checked ? 1 : 0;

          // Send update request to the server
          fetch("http://127.0.0.1:8000/update_gov_verified", {
            method: "POST",
            headers: {
